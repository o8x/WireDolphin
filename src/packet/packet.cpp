...
/*
 * TCP标志位
 * SYN（同步）：绿色，用于建立连接的初始请求
 * ACK（确认）：蓝色，用于确认收到的数据包
 * FIN（终止）：红色，用于终止连接的请求
 * RST（复位）：橙色，用于重置连接
 * PSH（推送）：紫色，用于提示接收方将数据立即提交给应用程序
 * URG（紧急）：黄色，用于标记紧急数据
 *
 * 三次握手
 * 第一次握手（SYN）：深绿， 客户端发送SYN请求，表示希望建立连接
 * 第二次握手（SYN-ACK）：天蓝， 服务器收到SYN后，回应SYN-ACK表示同意连接
 * 第三次握手（ACK）：深蓝， 客户端收到SYN-ACK后，发送ACK确认，连接建立
 *
 * 四次挥手
 * 第一次挥手（FIN）：深红， 客户端发送FIN请求，表示希望终止连接
 * 第二次挥手（ACK）：玫瑰， 服务器收到FIN后，发送ACK确认
 * 第三次挥手（FIN）：橙红， 服务器也发送FIN请求，表示同意终止连接
...
vector<int> Packet::get_color() const {
     ...
     * 第四次挥手（ACK）：金色，客户端收到FIN后，发送ACK确认，连接终止
     */
    if (this->flags->FIN && !this->flags->ACK) {
        return {255, 165, 0};
    }
